...
export default function ApartmentEditor({ user, sectorId, onBack, onCheckout }: { user: User, sectorId: string, onBack: () => void, onCheckout?: () => void }) {
    ...
    const actions = [
        { label: 'Sleep', icon: '💤', action: () => update(ref(db, `game_states/${user.uid}`), { isSleeping: true, sleepStartTime: Date.now() }) },
